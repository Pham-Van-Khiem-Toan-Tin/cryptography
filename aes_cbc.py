...
def xor_matrices(matrix1, matrix2):
    ...
    for r in range(4):
        for c in range(4):
            # Perform bitwise XOR on corresponding bytes
...
def aes_ecb_encrypt(plaintext, key, iv):
    ...
    ciphertext = []
    iv_text_bytes = string_to_hex(iv)
    previous_block = [[iv_text_bytes[j * 4 + i] for j in range(4)] for i in range(4)]
    # Bước 4: Mã hóa từng khối
    for block_index, block in enumerate(blocks):
        print(f"\n=== Mã hóa khối {block_index} ===")
        print(f"Khối plaintext (hex): {print_hex(block)}")
        # xor_block = [a ^ b for a, b in zip(block, previous_block)]
        # Chuyển khối thành ma trận trạng thái 4x4
        state = xor_matrices([[block[j * 4 + i] for j in range(4)] for i in range(4)], previous_block)
...
def main():
    ...
    # Mã hóa
    ciphertext_bytes = aes_ecb_encrypt(plaintext, key, iv)
